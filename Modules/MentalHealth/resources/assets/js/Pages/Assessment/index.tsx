...
import { User, Clipboard, Heart, Stethoscope, Calendar } from 'lucide-react';
...
export default function AssessmentIndex({ patient }: Props) {
  ...
  const nextStep = () => {
    if (currentStep === 0) {
      const errors: any = {};
      if (!physicalHealthData.assessment_physical_health) errors.assessment_physical_health = 'Assessment in Physical Health is required.';
      if (!physicalHealthData.management_physical_health) errors.management_physical_health = 'Management in Physical Health is required.';
  ...
  const handleSubmit = () => {
    ...
    const newAssessment = {
      ...
      icd_10_descrip: selectedDescrip,
    ...
    console.log('Submitting data:', {
      assessPhyHealthData: physicalHealthData,
      conMNSAssessData: MNSData,
      manMNSAssessData: manMNSData,
      diagMedsData: {
        diagnosis: selectedDiagnosis,
        icd_10_code: selectedIcdCode,
      ...
      scheduledDate: null, // or replace with actual state if you have one
    });
  
    router.post('/assessment/store', newAssessment, {
      onSuccess: () => {
        resetForm();
        router.visit(`/assessment/${patient.id}/addAssessment`);
      },
      onError: (errors) => {
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="p-4">
        ...
        {currentStep === 0 && (
          <AssessPhyHealth data={physicalHealthData} setData={setPhysicalHealthData} errors={physicalErrors} />
        )}
